...
export class DbChatsManagerService {
  ...
  async getDMChatroomsForUser(user: TbUa01MEntity) {
	...
	*/
    const results: TbCh01LEntity[] = []; // NOTE: 초기화 하지 않으면 에러 발생
    const prvChtrms: TbCh01LEntity[] = await this.ch01lRp.find({
      relations: {
        ch02lEntities: true,
      },
      where: {
    ...
    for (const prvChtrm of prvChtrms) {
      ...
      if (canUserSee === true) results.push(prvChtrm);
    }
    return results;
  }
  ...
  async isUserListedInThisChatroom(
    user: TbUa01MEntity,
    room: TbCh01LEntity,
  ): Promise<boolean> {
  ...
  async kickUserTransaction(
    target: TbUa01MEntity,
    room: TbCh01LEntity,
    targetInChtrm: TbCh02LEntity,
  ) {
    /*!SECTION
  ...
  async isUserBannedInARoom(user: TbUa01MEntity, room: TbCh01LEntity) {
    ...
    if (bannedUserInfo === null) return false;
    return true;
  }
